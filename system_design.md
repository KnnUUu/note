...
### 一个服务器上的一个1TB文件发送给全球分布的其他100个clients
- 使用checksum校验和检测传输过程中有没有错误或者被篡改，常用算法有MD5跟sha512sum，基本就是一个面向文件的hash func  
- 可以p2p但如果客户端不是公司自己的需要考虑中间有的客户端关闭  
- 可以设置多个DB然后replication  
- sharding文件  
